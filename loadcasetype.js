...
var getCaseType=()=>{
	return new Promise((resolve,reject) => {
		...
		var file_data = fs.readFileSync('./casetype.csv', { encoding : 'utf8'});
...
.then((casetypes)=>{
	...
	casetypes.forEach((casetype)=>{
		var addCaseType=(casetype)= new Promise((resolve, reject) => {
				apolloFetch({
					query: `mutation createCaseType ($type: String!) 
                            {
                                createCaseType (type: $type) {
                                    type
                                }
                            }`,
					variables: { 
					...
					resolve(res.data.createCaseType.id)
				})
				.catch((err)=>{
					reject(err);
				});
		})
		addCaseTypedata.push(addCaseType)
	})
	Promise.all(addCaseTypedata)
	.then(()=>{
		console.log("Done")
	})
	.catch((err)=>{
